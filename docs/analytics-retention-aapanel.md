...
Audience Overview and Devices now record and read MySQL visitor statistics. Browser keys are deduplicated per day; the 30-day total uses COUNT(DISTINCT visitor_key), not a sum of daily totals. Devices may overlap across days. Dashboard results are cached for five minutes. Redis is not configured, no .env is added, and database connection settings are unchanged.
...
The command retains today and the preceding 29 calendar dates, using the application's existing timezone. For example, on 2026-09-07 it deletes visit_date before 2026-08-09. It cleans traffic_daily_visitors and traffic_daily_player_metrics, plus analytics_daily if that table remains from an earlier deployment. Missing tables are skipped. Run migrations to ensure traffic_daily_visitors exists. Existing rows within the 30-day window are included.
