...
VidHide/EarnVids HTTP/API 404 atau 410 juga melewati host tanpa menganggapnya pasti Deleted. Status 404/410 pada record file dengan ID yang cocok tetap Deleted. Error favicon `manifest.json` bukan status file video. Jika iframe terbuka tetapi stream internal macet karena CORS/timeout, tombol **Ganti server** mencoba host berikutnya (termasuk tanpa API) untuk sesi penonton tersebut. Tombol ini tidak menandai file terhapus. Browser induk tidak dapat membaca error internal iframe lintas domain secara langsung.
