...
(async()=>{
 ...
 try {
  ...
  await page.route('https://provider.example/**',route=>route.fulfill({contentType:'text/html',body:'Provider'}));
  ...
  const result=await page.evaluate(()=>{
   ...
   Player.loadFrame('https://provider.example/video');
   timeout(); timeout();
  ...
  const shortDeadline=await page.evaluate(()=>{
   ...
   Player.loadFrame('https://provider.example/first');
   Player.activeLinkId='three'; Player.loadFrame('https://provider.example/second');
