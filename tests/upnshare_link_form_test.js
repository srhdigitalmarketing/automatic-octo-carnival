...
(async () => {
 ...
 try {
  ...
  await page.setContent(`<div id="st-group-content"><div class="st-group"><label>Link 1</label><input class="link" name="st_links[1][url]" value="https://host/e/old123" readonly><input class="stream-priority" name="st_links[1][host_priority]" value="200"><input type="hidden" name="st_links[1][id]" value="42"><input type="hidden" name="st_links[1][upnshare_video_id]" value="old123"><span class="stream-server-badge is-broken">Deleted</span><button class="clone-st-group">+</button></div></div>`);
  ...
  assert.equal(await clone.locator('select').count(),0);
  ...
  assert.equal(await groups.first().locator('.link').inputValue(),'https://host/e/old123');
  console.log('PASS: cloned link resets video identity and badge without an account dropdown without modifying original.');
