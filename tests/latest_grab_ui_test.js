...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await page.setContent('<div id="latest-grab-panel" data-url="https://example.test/run"><select id="latest-grab-category"><option value="">All</option></select><button id="latest-grab-categories">Load categories</button><input id="latest-grab-count" value="5"><select id="latest-grab-api"><option value="1">Catalog</option></select><button id="latest-grab-start">Start</button><button id="latest-grab-stop">Stop</button><progress id="latest-grab-progress"></progress><p id="latest-grab-status"></p><ul id="latest-grab-log"></ul></div>');
