...
document.addEventListener('click',async event=>{
 ...
 const controls=Array.from(document.querySelectorAll('#reported-host-filter button, #reported-host'));
 ...
 let processed=0;const parts=['\uFEFFJudul,Link Error\r\n'];
 try{
  ...
  while(!stopped&&!done){
   ...
   if(!clear)parts.push(result.rows.map(row=>cell(row.title)+','+cell(row.link)+'\r\n').join(''));
  ...
  if(!stopped){progress(100);if(!clear){const url=URL.createObjectURL(new Blob(parts,{type:'text/csv;charset=utf-8;'}));const a=document.createElement('a');a.href=url;a.download='error-links-'+(selected||'all-hosts')+'.csv';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}}
  status.textContent=stopped?`Dihentikan. ${processed} diproses.${clear?' Clear yang selesai tetap tersimpan.':' Export dibatalkan, file tidak diunduh.'}`:`Selesai: ${processed} ${clear?'laporan dibersihkan.':'link diexport ke CSV.'}`;
