...
<div class="host-api-overview">
    ...
    <a href="<?= admin_url('/third-party-apis/new?provider=upnshare') ?>" class="btn btn-primary">Add UPNShare</a>
        <a href="<?= admin_url('/third-party-apis/new?provider=vidhide') ?>" class="btn btn-primary">Add EarnVids</a>
    ...
    <div class="x_content">
        <table class="table host-api-table">
            ...
            <tr>
                ...
                <td><span class="host-api-provider-badge"><?= $api->provider === 'vod_catalog' ? 'VOD title search' : (in_array($api->provider, ['upnshare','vidhide','custom_http'], true) ? ($api->provider === 'custom_http' ? 'Custom hostname' : ($api->provider === 'vidhide' ? 'EarnVids' : 'UPNShare')) : 'Cloudflare R2') ?></span></td>
