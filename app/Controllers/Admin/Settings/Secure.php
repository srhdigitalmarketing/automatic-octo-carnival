...
use App\Libraries\SecureBackupRemote;
class Secure extends BaseSettings
{
    public function index()
    {
        ...
        $remote=[];
        try { $remote=(new SecureBackupRemote())->settings(); }
        catch (\Throwable $exception) { $error='Pengaturan tujuan remote tidak dapat dibaca. Periksa file kunci dan izin writable.'; }
        return view('admin/settings/secure',['title'=>'Backup','remote'=>$remote,'entries'=>$entries,'error'=>$error,'token'=>session()->get('secure_backup_token')]);
    }
    public function run()
    {
        $this->response->setHeader('Cache-Control','no-store');
        if (strtolower($this->request->getMethod())!=='post' || !hash_equals((string)session()->get('secure_backup_token'),(string)$this->request->getPost('token')) || !session()->get('secure_backup_token')) return $this->response->setStatusCode(403)->setJSON(['message'=>'Sesi tidak valid. Muat ulang halaman Backup.']);
        ...
        try {
            ...
            $destination=(string)($this->request->getPost('destination') ?: 'local');
            $before=[];
            if (in_array($action,['files','database'],true) && $destination!=='local') {
                (new SecureBackupRemote())->ready($destination);
            ...
            switch ($action) {
                ...
                case 'remote-send':
                    ignore_user_abort(true);
                    (new SecureBackupRemote())->send($store,(string)$this->request->getPost('id'),$destination);
                    $message='Backup berhasil dikirim ke '.$destination.'. Salinan lokal tetap tersedia.';
                    break;
            ...
            if (in_array($action,['files','database'],true) && $destination!=='local') {
                $created=array_values(array_filter($store->entries(),static fn($item)=>!in_array($item['id'],$before,true)));
                if (count($created)!==1) throw new \RuntimeException('Backup lokal selesai tetapi arsip untuk pengiriman tidak dapat ditentukan. Gunakan Kirim pada daftar backup.');
    ...
    public function download()
    {
        ...
        return $this->response->download($path,null)->setFileName('backup-'.date('Ymd-His').'.'.pathinfo($path,PATHINFO_EXTENSION))->setHeader('Cache-Control','private, no-store')->setHeader('X-Content-Type-Options','nosniff');
    }
}
