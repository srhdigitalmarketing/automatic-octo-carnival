...
class Homepage extends BaseSettings
{
    ...
    {
        return redirect()->to(admin_url('/settings/site#index-homepage'));
    }
    public function update()
    {
        if (!$this->validate(['homepage_active'=>'required|in_list[0,1]', 'homepage_heading'=>'required|max_length[160]', 'homepage_intro'=>'required|max_length[500]'])) {
            return redirect()->to(admin_url('/settings/site#index-homepage'))->withInput()->with('errors',$this->validator->getErrors());
        }
        ...
        if (!$db->transStatus()) { $db->transRollback(); return redirect()->to(admin_url('/settings/site#index-homepage'))->with('errors',['Pengaturan gagal disimpan.']); }
        $db->transCommit();
        return redirect()->to(admin_url('/settings/site#index-homepage'))->with('success','Homepage settings saved.');
    }
}
