...
class ThirdPartyApis extends BaseController
{
    ...
    {
        $api = $this->model->where('id', $id)->whereIn('provider', ['cloudflare_r2', 'upnshare', 'custom_http', 'vod_catalog'])->first();
    ...
    private function providerData(array $data): array
    {
        ...
        $fields = in_array($data['provider'], ['upnshare','custom_http'], true)
    ...
    {
        if (!in_array($data['provider'] ?? '', ['upnshare','custom_http','vod_catalog'], true) || ($data['status'] ?? 'active') !== 'active') { return []; }
        $domains = preg_split('/[\s,]+/', strtolower(trim((string)($data['embed_domains'] ?? ''))), -1, PREG_SPLIT_NO_EMPTY);
        foreach ($this->model->whereIn('provider', ['upnshare','custom_http','vod_catalog'])->where('status', 'active')->findAll() as $api) {
    ...
    private function providerErrors(array $data): array
    {
        ...
        if (!in_array($data['provider'] ?? '', ['upnshare','custom_http'], true)) { return $this->r2Errors($data); }
        $errors = [];
